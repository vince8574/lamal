<div @class([
    ...
    <div class='w-full'>
        <input name='name' placeholder="Entrez votre joli nom"
            class='bg-white rounded-[10px] border border-gray-300 w-full py-2 pr-3 pl-6 font-roboto font-bold text-[22px]'
            wire:model="name">
        @error('name')
            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
        @enderror
    ...
    <div class="w-full flex justify-end">
        <a wire:click="createProfile"
            class='rounded-[10px] bg-customYellow h-auto py-2 px-6 text-center justify-center flex items-center justify-center font-bold text-[22px] gap-4 cursor-pointer'>
            Comparer
